...
def generate_pdf(study_hours, attendance, previous_marks, prediction):
    ...
    c.drawString(100, 660, f"Previous Marks: {previous_marks}")
...
def save_to_db(data):
    ...
    (study_hours, attendance, previous_marks, predicted_score)
    VALUES (?, ?, ?, ?)
""", data)
    conn.commit()
    conn.close()
...
if page == "🏠 Home":
    ...
    if st.button("Predict"):
        ...
        # ---------------- VALIDATION ----------------
        if study_hours < 0 or attendance < 0 or previous_marks < 0:
            st.error("❌ Values cannot be negative!")
        elif attendance > 100:
            st.error("❌ Attendance cannot exceed 100%!")
        elif previous_marks > 100:
            st.error("❌ Previous marks cannot exceed 100!")
        else:
            ...
            # ---------------- SAVE ----------------
            save_to_db(
                (study_hours, attendance, previous_marks, prediction)
            )
            ...
            if prediction >= 90:
                grade = "A"
                st.success("🏆 Grade A - Excellent Performance")

            elif prediction >= 75:
                grade = "B"
                st.info("👍 Grade B - Good Performance")

            elif prediction >= 60:
                grade = "C"
                st.warning("📚 Grade C - Average Performance")

            else:
                grade = "D"
                st.error("⚠️ Grade D - Needs Improvement")

        st.markdown(f"### 🎯 Final Grade: {grade}")
        # ---------------- PERFORMANCE INSIGHTS ----------------

        if prediction >= 90:
            st.success("🏆 Excellent Performance! Keep maintaining your consistency.")

        elif prediction >= 70:
            st.info("👍 Good Performance! You are doing well, but there is still room for improvement.")

        else:
            st.warning("⚠️ Needs Improvement. Increase study discipline and improve learning habits.")
        ...
        - Previous marks heavily influence the result
        """)
# ---------------- HISTORY PAGE ----------------
